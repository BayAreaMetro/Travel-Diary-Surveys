...
trip              <- read.csv(trip_in) %>%
  ...
  mutate(express_lane_use=if_else(
...
# i680_80_to_580=1 if i680_80_to_bm_bridge==1, bm_bridge==1,and/or i680_bm_bridge_to_580==1
...
# Use select to order facilities
...
final <- sum_links %>% 
  ...
  mutate(sr37_80_to_101=if_else((sr37_80_to_mare==1 | sr37_mare_to_121==1 | sr37_121_to_101==1),1,0),
         ...
         i680_80_to_580=if_else((i680_80_to_bm_bridge==1 | bm_bridge==1 | i680_bm_bridge_to_580==1),1,0),
...
trips_with_weights <- final %>%
  left_join(.,trip %>% select(trip_id,trip_weight_rmove_only),by="trip_id") %>% 
