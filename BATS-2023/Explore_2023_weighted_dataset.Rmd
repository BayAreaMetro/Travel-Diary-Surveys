...
household_county_bats <- household %>% 
  ...
  summarize(Total_BATS=round(sum(hh_weight))) %>% 
  ungroup() %>% 
...
person_county_bats <- person %>%
  ...
  group_by(home_county) %>% 
  summarize(Total_BATS=round(sum(person_weight))) %>% 
  ungroup() %>% 
  rename(County=home_county)
