...
adults <- left_join(person,weight, by="person_id") %>%    # Include only persons with a weight value 
  ...
  select(person_id,hh_id,County_Name, wt_alladult_WkDay,home_county_fips,incomerc,racerc,agerc,num_peoplerc,dummy)
...
racesum1 <- adults %>% 
  filter(wt_alladult_WkDay>0) %>% 
  group_by(County_Name,racerc) %>% 
  summarize(total=sum(dummy)) %>% 
  spread (., racerc, total, fill=0)

write.csv(racesum1, "TNC_Ethnicity_Unweighted.csv", row.names = FALSE, quote = T)
