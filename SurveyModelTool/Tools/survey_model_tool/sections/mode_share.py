...
class ModeShareSectionBuilder:
    def __init__(self, plotter, config):
        ...
        if isinstance(config, str):
            ...
            self.config = config
        self.plotter.data_source = self.plotter.data_source[self.plotter.data_source[self.config['global']['columns']['mode']].isin(self.config['sections']['mode_share']['axis_mode'])]  # Filter out rows with NA mode
    ...
    def build(self, ):
        ...
        fig_by_income_stack = self.plotter.plot_stacked_bar(
            df=self.plotter.data_source[self.plotter.data_source[self.config['global']['columns']['income_label']].isin(self.config['sections']['mode_share']['income_order'])], add_ref=True,
        ...
        purpose_mode = self.plotter.plot_grouped_bar_with_dropdown(
            df=self.plotter.data_source, axis_order=self.config['sections']['mode_share']['axis_mode'], group_col=self.config['global']['columns']['mode'],
