...
class Plotter:
    ...
    def summary_bar(self,survey, stats = ['population', 'households', 'trips', 'tours'], weighted = True, trip_weight = 'trip_weight', hh_weight = 'hh_weight', person_weight = 'person_weight', tour_weight = 'tour_weight'):
        ...
        if survey.tours is not None:
            ...
            tour_div = self.stat_bubble(label = f"{heading}Tours", value = tour_count, icon_svg = 'tour')
        

        pop_div = self.stat_bubble( label = f"{heading}Population", value = population_count, icon_svg ='population')
        hh_div = self.stat_bubble( label = f"{heading}Households", value = household_count, icon_svg = 'household')
        trip_div = self.stat_bubble( label = f"{heading}Trips", value = trip_count, icon_svg = 'trip')
        if survey.tours is not None:
